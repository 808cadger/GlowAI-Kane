  ...
  async function call (apiKey, payload, opts = {}) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      if (attempt > 0) {
        ...
        log.warn('glowai-api retry', { reqId, attempt, waitMs: Math.round(wait) })
      ...
      try {
        log.info('glowai-api request', { reqId, attempt, model: payload.model })
        ...
        if (!res.ok) {
          ...
          log.warn('glowai-api http-error', { reqId, status: res.status, latencyMs, msg })
        ...
        cbSuccess()
        log.info('glowai-api ok', { reqId, latencyMs, stopReason: data.stop_reason })
        return data
